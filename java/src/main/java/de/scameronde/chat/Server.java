...
public class Server {
  private static Repository repository = new InMemoryRepository();
  ...
  public static void main(String[] args) {
    ...
    get("/chatRoom/:chatRoomId", ((request, response) -> {
      ...
      Option<ChatRoom> chatRoom = findChatRoom(id);
      Option<MessageLog> messageLog = chatRoom.map(cr -> repository.getMessageLog(cr));
      Either<Exception, Option<String>> jsonMessageLog = dataToJson(messageLog);
    ...
    delete("/chatRoom/:chatRoomId", ((request, response) -> {
      String id = getParameter(request, ":chatRoomId");
      Option<ChatRoom> chatRoom = findChatRoom(id);
      chatRoom.forEach(cr -> repository.deleteChatRoom(cr));
      Either<Exception, Option<String>> result = Either.right(chatRoom.map(x -> ""));
    ...
    get("/participant/:participantName", ((request, response) -> {
      ...
      Option<Participant> knownParticipant = repository.login(participantName);
      Either<Exception, Option<String>> result = dataToJson(knownParticipant);
      return createOptionalResponse(response, result);
    }));
  ...
  private static Option<ChatRoom> findChatRoom(String id) {
    return repository.getChatRooms().find(room -> room.getId().equals(id));
  }
  ...
  private static String createOptionalResponse(Response response,
                                               Either<Exception, Option<String>> payload) {
    if (payload.isRight()) {
      Option<String> payloadData = payload.get();
      if (payloadData.isDefined()) {
        setResponseOK(response);
  ...
  private static String getParameter(Request request, String parameter) {
